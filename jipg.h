...
static void jipg_emit_array_parser(FILE *source, Jipg_Value *array) {
    ...
    const Jipg_Value *internal = array->as_array.internal;
    fprintf(source,
            "static inline bool parse_%s(Lexer *l, %s *res) {\n"
...
static void jipg_emit_source(FILE *source, Jipg_Value **values, size_t value_count) {
...
static int jipg_main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; i < value_count; ++i) {
        ...
        values[i]->head = jipg_global_context.parsers[i].head_struct_name;
        jipg_generate_struct_names(
            values[i],
            jipg_global_context.parsers[i].head_struct_name);
    }
    ...
    jipg_emit_source(source, values, value_count);
